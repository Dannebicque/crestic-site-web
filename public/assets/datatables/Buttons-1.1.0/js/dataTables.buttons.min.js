...
(function (e) {
	...
	var j = e.fn.dataTable, s = 0, t = 0, k = j.ext.buttons, l = function (a, b) {
		...
		this.s = {dt: new j.Api(a), buttons: [], subButtons: [], listenKeys: "", namespace: "dtb" + s++};
	...
	e.extend(l.prototype, {
		action: function (a, b) {
			...
			c = 0;
			for (a = b.length; c < a; c++)if (b.inst === this) {
				b.splice(c, 1);
				break
			}
			return this
			...
			c = 0;
			for (a = b.length; c < a; c++)for (d = b[c].length - 1; 0 <= d; d--)null === b[c][d] && (b[c].splice(d, 1), this.c.buttons[c].buttons.splice(d, 1));
			return this
		}, removePrep: function (a) {
			var b, c = this.s.dt;
			if ("number" === typeof a || -1 === a.indexOf("-"))b = this.s.buttons[1 * a], b.conf.destroy && b.conf.destroy.call(c.button(a), c, b, b.conf), b.node.remove(), this._removeKey(b.conf), this.s.buttons[1 * a] = null; else {
				var d = a.split("-");
				b = this.s.subButtons[1 * d[0]][1 * d[1]];
				b.conf.destroy && b.conf.destroy.call(c.button(a),
					c, b, b.conf);
				b.node.remove();
			...
			var c = this._indexToButton(a), d = this.c.dom.collection.buttonLiner, d = "string" === typeof a && -1 !== a.indexOf("-") && d && d.tag ? d.tag : this.c.dom.buttonLiner.tag, e = this.s.dt, g = function (a) {
			...
			b = 0;
			for (c = d.length; b < c; b++)if (d[b].node[0] === a)return b + "";
			b = 0;
			for (c = g.length; b < c; b++) {
				d = 0;
				for (e = g[b].length; d < e; d++)if (g[b][d].node[0] === a)return b + "-" + d
			}
			...
			for (var f = 0, g = a.length; f < g; f++) {
				...
				if (h)if (e.isArray(h))this._buildButtons(h, b, c); else {
			...
			var h = this.s.subButtons, i = function (c, d) {
				if (c.key)if (c.key === a)d.click(); else if (e.isPlainObject(c.key) && c.key.key === a && (!c.key.shiftKey || b.shiftKey))if (!c.key.altKey || b.altKey)if (!c.key.ctrlKey || b.ctrlKey)(!c.key.metaKey || b.metaKey) && d.click()
			};
			c = 0;
	...
	l.instanceSelector = function (a, b) {
		...
		var c = [], d = e.map(b, function (a) {
			...
			if (e.isArray(a))for (var h = 0, i = a.length; h < i; h++)f(a[h]); else"string" === typeof a ? -1 !== a.indexOf(",") ? f(a.split(",")) : (a = e.inArray(e.trim(a), d), -1 !== a && c.push(b[a].inst)) : "number" === typeof a && c.push(b[a].inst)
		};
		f(a);
		return c
	};
	l.buttonSelector = function (a, b) {
		for (var c =
			...
			e.each(b.s.buttons, function (a, b) {
				null !== b && j.push({node: b.node[0], name: b.name})
			});
			...
			if (e.isArray(a) || a instanceof e) {
				...
				f = 0;
				for (g = j.length; f < g; f++)c.push({inst: b, idx: b.toIndex(j[f].node)})
			...
			} else if (a.match(/^\d+(\-\d+)?$/))c.push({inst: b, idx: a}); else if (-1 !== a.indexOf(":name")) {
				k = a.replace(":name", "");
				f = 0;
				for (g = j.length; f < g; f++)j[f].name === k && c.push({inst: b, idx: b.toIndex(j[f].node)})
			} else e(f).filter(a).each(function () {
				c.push({inst: b, idx: b.toIndex(this)})
	...
	e.extend(k, {
		collection: {
			...
			var a = a.settings()[0].aLengthMenu, b = e.isArray(a[0]) ? a[0] : a, c = e.isArray(a[0]) ? a[1] : a, d = function (a) {
	...
	j.Api.register("buttons.exportData()", function (a) {
		if (this.context.length) {
			...
			return {header: a, footer: f, body: k}
		}
	});
	var r = e("<textarea/>")[0];
	...
	j.ext.feature.push({
		fnInit: function (a) {
			var a = new j.Api(a), b = a.init().buttons || j.defaults.buttons;
